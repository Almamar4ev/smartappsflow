...
self.addEventListener('fetch', e => {
  ...
  // ─── HTML: Network First بمهلة قصوى (تحديث فوري بلا بطء) ───
  // ننتظر الشبكة 2.5 ثانية كحد أقصى؛ لو تأخرت أو انقطعت نعرض الكاش فوراً.
  // أي نسخة أحدث تصل لاحقاً تُخزَّن، ويتكفّل شريط التحديث بإبلاغ المستخدم.
  if (isHtmlRequest(req)) {
    e.respondWith((async () => {
      ...
      if (!cached) {
        // أول زيارة: لا خيار سوى الشبكة
        return netPromise.catch(() => new Response('offline', { status: 503 }));
      }
      const timeout = new Promise(resolve => setTimeout(() => resolve(null), 2500));
      const winner = await Promise.race([netPromise.catch(() => null), timeout]);
