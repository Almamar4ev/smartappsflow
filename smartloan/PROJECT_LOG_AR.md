...
**الحل النهائي المطبّق (نهج مختلف جذريًا — حشوة أصلية):**
- في `MainActivity` (يُولّده ملف البناء): نضع مستمع insets على `android.R.id.content`، ونطبّق
  الحشوة **أصليًا** عبر `v.setPadding(insets)` ونُرجع `WindowInsetsCompat.CONSUMED`. Android
  يطبّقها في دورة التخطيط مباشرة — **بلا JS ولا CSS ولا توقيت** — فلا يتداخل المحتوى أبدًا.
- خلفية منطقة الأشرطة `#F1F5F9` (سطح فاتح يطابق الثيم الفاتح).
- في `applyStatusBar`: `setOverlaysWebView({overlay:true})` (أشرطة شفافة تُظهر منطقة الحشوة).
...
- [ ] **لون أشرطة النظام حسب الثيم:** الحل الحالي يجعل الأشرطة فاتحة دائمًا (#F1F5F9). ممتاز
      للثيم الفاتح، لكن في الثيمات الداكنة ستكون الأشرطة فاتحة (وربما أيقونات النظام غير واضحة).
      لجعلها تتبع الثيم يلزم جسر لون من JS إلى الكود الأصلي (مثلًا عبر JavascriptInterface يضبط
      `content.setBackgroundColor` + `window.setStatusBarColor/setNavigationBarColor`).
